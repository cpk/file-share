...
String emailCheck = String.valueOf(session.getAttribute("iduser"));
	...
	Query queryFolder;
	if(typeSort==0){
		queryFolder = new Query("folder", logKeyFolder).addSort(keySort, Query.SortDirection.ASCENDING);
	}else{
		queryFolder = new Query("folder", logKeyFolder).addSort(keySort, Query.SortDirection.DESCENDING);
	}
...
<%
	if(!foldersFolder.isEmpty()) {
		...
		for(int i=min; i<max; i++){
			...
			%>
            <form action="index.jsp?form_folders" method="post" name="edit">
            ...
            <form action="/deleteFolder" method="post" name="delete">
